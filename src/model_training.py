    ...
    def train(self):
        try:
            ...
            for epoch in range(self.epochs):
                ...
                # ป้อน image และ target ให้ model
                for i, (images, targets) in enumerate(train_loader):
                    self.optimizer.zero_grad()
                    losses = self.model(images, targets)
                    
                    if isinstance(losses, dict):
                        # รวมค่า loss ทั้งหมด (เพราะ Faster R-CNN มี loss หลายตัว)
                        total_loss = 0
                    ...
                    total_loss.backward()
                    self.optimizer.step()
                self.writer.flush() # write any pending events or summaries from its internal buffer to the designated log file
